...
def search_config_in_configuration_collection(config_name):
    print ("Searching config blob %s in configuration collection" % config_name)
    mongo_ap_collection = retrieve_collection(properties.configuration_collection_name)
    mongo_ap_document = mongo_ap_collection.find_one({"config_name": config_name})
    mongo_ap_document = (mongo_ap_document)
    if mongo_ap_document != None:
        print ("Config %s is found in configuration collection." % config_name)
    else:
        raise AssertionError("Config %s is not found in configuration collection." % config_name)
...
def verify_customer_default_config_id(config_id):
    mongo_customer_collection = retrieve_collection(properties.customer_collection_name)
    mongo_customer_document = mongo_customer_collection.find_one({"name": properties.customer_name}, {"default_config_id": 1})
    default_config_id = str(mongo_customer_document['default_config_id'])
    if default_config_id == str(config_id):
        print ("Config %s is pushed to customer %s successfully." % (properties.configuration_name, properties.customer_name))
    else:
        raise AssertionError("Config %s is not pushed to customer %s." % (properties.configuration_name, properties.customer_name))
